...
# Get One page from the api, convert that to text using json method and return
def fetch_playlist_item_page(google_api_key, youtube_playlist_id, pagetoken=None):
...
# Fetch the playlist items using the nextpage token if there exists a next page and recursively call itself to keep fetching
# Yield is a generator that keeps streaming in data if available
def fetch_playlist_items(google_api_key, youtube_playlist_id, page_token=None):
    ...
    next_page_token = payload.get("nextPageToken", None)
    if next_page_token:
        yield from fetch_playlist_items(google_api_key, youtube_playlist_id, next_page_token)


def fetch_videos_page(google_api_key, video_id, pagetoken=None):
